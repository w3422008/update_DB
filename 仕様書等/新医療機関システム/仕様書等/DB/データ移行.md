...
| status         | op_flg        | `active/closed/locked` 開院/閉院/編集不可状態 |
| bed_count      | bed           | 許可病床数 |
| consultation_hour | con_hour   | 診療時間 |
| has_carna_connect | ---        | カルナコネクト（元データは、テーブル内データの有無の為、あれば1） |
| has_pt         | pt            | boolean変換 |
| has_ot         | ot            | boolean変換 |
| has_st         | st            | boolean変換 |
| notes          | note          | 備考 |
...
## 14. 連携パスマスタ（clinical_paths）

- **移行先テーブル**: `clinical_paths`
- **主キー**: `clinical_path_id`
- **移行元テーブル**: `c_path`のカラム定義から
- **カラム対応表**:

| 移行先カラム | 移行元カラム | 備考 |
|-------------|-------------|------|
| clinical_path_id | --- | 'CP001'〜'CP009' |
...
## 15. 医療機関-連携パス関連（clinical_path_hospitals）

- **移行先テーブル**: `clinical_path_hospitals`
- **主キー**: 複合キー（hospital_id, clinical_path_id, facility_id）
- **移行元テーブル**: `c_path`
- **カラム対応表**:
...
|-------------|-------------|------|
| hospital_id | hos_cd | FK:hospitals |
| clinical_path_id | --- | cp0〜cp8から判定 |
| facility_id | ins | 施設ID（0:附属 1:総合） |

*備考: cp0〜cp8の値が1の場合、対応するパスを実施*

---

## 16. 川崎学園施設マスタ（kawasaki_university_facilities）
...
## 17. 川崎学園部署マスタ（kawasaki_university_departments）
...
| department_id | bel | 部署ID取得後、該当するマスタデータを登録 |
...
- **移行先テーブル**: `referrals`
- **主キー**: `hospital_id`,`facility_id`,`year`,`date`,`department_name`,`intro_type`
...
| hospital_id | hos_cd | 医療機関コード |
| facility_id | ins | 施設ID（0:附属 1:総合） |
| referral_type | --- | 'intro'/'inverse_intro' |
| year | year | 年度 |
| date | date | 診療日 |
| department_name | fie_name | 診療科名 |
| department_id | fie_cd | 診療科コード |
| referral_count | intr, invr_intr | 件数 |
...
- **移行先テーブル**: `training`
- **主キー**: `hospital_id`,`year`,`facility_id`,`department`,`start_date`
- **移行元テーブル**: `training`
- **カラム対応表**:
...
- **移行先テーブル**: `contacts`
- **主キー**: `hospital_id`,`year`,`facility_id`,`date`,`method`
- **移行元テーブル**: `contact`
- **カラム対応表**:
...
| resolution | answer | 回答内容 |
| created_at | order_date | 問い合わせ日時 |
| updated_at | --- | 回答日 |
| resolved_at | ans_date | 対応終了日 |

*備考: trash_flgが1のデータは移行対象外*

---

## 25. メンテナンス通知（maintenances）
...
## 26. メンテナンス実行中通知（maintenance_start）
...
## 27. システムバージョン管理（system_versions）
...
## 31. 医療連携懇話会参加年度（social_meetings）

- **移行先テーブル**: `social_meetings`
- **主キー**: 複合キー（hospital_id, facility_id, meeting_year）
...
## 32. 医療機関コード履歴（hospital_code_history）
