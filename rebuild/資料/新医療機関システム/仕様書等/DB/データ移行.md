...
| status         | onf  　       | `active/closed` |
| bed_count      | bed           | 許可病床数 |
| bed_rehabilitation | bed_reh   | リハビリ病床 |
| bed_medical_care | bed_tre     | 療養病床 |
| bed_general    | bed_main      | 一般病床 |
| bed_nursing_care | bed_care    | 介護病床 |
| bed_conversion | bed_tra       | 転換病床 |
| bed_psychiatric | bed_att      | 精神病床 |
...
- **移行元テーブル**: （例：`old_address`）

| 移行先カラム | 移行元カラム | 備考 |
|--------------|-------------|------|
| hospital_id  | hospital_code | FK:hospitals |
| area_id      | area_code   | FK:areas |
| postal_code  | zip_code    |  |
| street_number| street      | 番地 |
...
| ip_address | --- | NULL |
| facility_id | ins | 施設ID |
| department_id | bel | 部署ID |
| is_admin | adm_user | 管理者フラグ |
| created_at | --- | 現在日時 |
...
| department_code | fie_cd | 診療科コード |
| intro | intr, invr_intr | 件数 |
...
- **移行先テーブル**: `positions`
- **主キー**: `position_id`
- **移行元テーブル**: `positions`, `training`のpositionから
- **カラム対応表**:
...
- **移行先テーブル**: `training`
- **主キー**: なし
- **移行元テーブル**: `training`, `training_backup`
- **カラム対応表**:
...
- **移行先テーブル**: `contacts`
- **主キー**: なし
- **移行元テーブル**: `contact`, `contact_backup`
- **カラム対応表**:
...
## 29. メッセージ・要望管理（message）

- **移行先テーブル**: `message`
- **主キー**: `message_id`（AUTO_INCREMENT）
- **移行元テーブル**: `message`, `message_old`
- **カラム対応表**:

| 移行先カラム | 移行元カラム | 備考 |
|-------------|-------------|------|
| status | situation | ステータス変換 |
| comment | comment | 内容 |
| view | view | 表示フラグ |
| version_id | version | バージョンとリンク |
| assigned_to | --- | デフォルトユーザー設定 |
| priority | --- | デフォルト'normal' |
| res_date | res_date, date | 対応日 |
| created_at | req_date, date | 作成日時 |
...
- **移行先テーブル**: なし（新システムでは未使用）
- **主キー**: `rel_cd`
- **移行元テーブル**: `relative`
- **備考**: 新システムでは親族情報管理機能は廃止

---

## 32. 医療連携懇話会参加年度（social_meeting）
...
- **移行先テーブル**: なし（areasテーブルに統合）
- **主キー**: なし
- **移行元テーブル**: `med_ass`
- **備考**: 医師会情報は地域マスタに統合して管理
