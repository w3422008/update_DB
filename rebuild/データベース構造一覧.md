...
| closure_date | date | NULL | 閉院日 |
| established_date | date | NULL | 開設日 |
...
| notes | text | NULL | 備考 |
| start_date | date | NULL | 就任日 |
| end_date | date | NULL | 退任日 |
...
| period | enum('morning','afternoon','evening','night') | NOT NULL | 時間帯 |
...
| フィールド名 | データ型 | 制約 | 説明 |
|-------------|----------|------|------|
| service_id | int(11) | PRIMARY KEY AUTO_INCREMENT | 診療内容ID |
| service_code | varchar(10) | UNIQUE NOT NULL | 診療内容コード |
| service_name | varchar(300) | NOT NULL | 診療内容名 |
| service_category | varchar(100) | NULL | 診療カテゴリ |
...
| relation_id | int(11) | PRIMARY KEY AUTO_INCREMENT | 関連ID |
| hospital_id | varchar(10) | FOREIGN KEY | 医療機関コード |
| service_id | int(11) | FOREIGN KEY | 診療内容ID |
| availability_level | tinyint(1) | DEFAULT 1 | 提供レベル（1-3） |
...
#### `external_connections` テーブル（外部システム連携）
外部システムとの連携情報を統合管理

| フィールド名 | データ型 | 制約 | 説明 |
|-------------|----------|------|------|
| connection_id | int(11) | PRIMARY KEY AUTO_INCREMENT | 連携ID |
| hospital_id | varchar(10) | FOREIGN KEY | 医療機関コード |
| system_name | varchar(50) | NOT NULL | システム名 |
| connection_status | enum('active','inactive','pending') | DEFAULT 'pending' | 連携状況 |
| connected_date | date | NULL | 連携開始日 |
| notes | text | NULL | 備考 |
| is_deleted | boolean | DEFAULT false | 削除フラグ |

**旧テーブルマッピング（carna_connect→external_connections）:**
- `hos_cd` → `hospital_id`
- `'carna_connect'` → `system_name`
- `delete_flg` → `is_deleted`
